...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/06/15 19:28:44 by nzhuzhle          #+#    #+#             */
/*   Updated: 2023/06/15 19:59:36 by nzhuzhle         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || fd > OPEN_MAX || BUFFER_SIZE <= 0)
		return (NULL);
	read_to_buf(fd, &list[fd], &lenth);
	if (list[fd] == NULL)
		return (NULL);
	str = malloc(lenth + 1);
	if (str == 0)
		return (free_all(&list[fd], NULL));
	add_to_string(list[fd], str, lenth);
	if (clean_list(&list[fd]) < 0)
		return (free_all(&list[fd], str));
	if (*str == '\0')
		return (free_all(&list[fd], str));
	return (str);
}
